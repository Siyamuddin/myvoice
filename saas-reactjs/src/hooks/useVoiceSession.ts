...
/**
 * Event handlers for inbound voice session frames.
 * Pass to `connect(handlers)`; the hook keeps a stable ref so callbacks can change.
 */
...
/**
 * Manages the authenticated WebSocket voice session: binary audio I/O,
 * JSON frame dispatch, heartbeat RTT, and exponential-backoff reconnect.
 *
 * Usage:
 * ```ts
 * const { state, rttMs, connect, disconnect, sendAudio } = useVoiceSession()
 * connect({
 *   onAudio: (pcm) => player.playChunk(pcm),
  ...
  const handleMessage = useCallback((event: MessageEvent) => {
    ...
    switch (type) {
      ...
      case 'error': {
        ...
        const message = typeof frame.message === 'string' ? frame.message : 'Voice session error';
